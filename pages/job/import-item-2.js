import Header from "../../components/header";
import Footer from "../../components/footer";
import {toggleFunc} from "../../components/clickFunc";

export default function () {
  return (
    <>
      <Header/>
      <div className="section job-post wf-section">
        <div className="container-default w-container">
          <div className="backlink-wrapper"><a href="/" className="backlink"><span
            className="backlink-arrow"></span>&nbsp;&nbsp;&nbsp;See all trials</a></div>
          <div className="job-post-wrapper">
            <div data-w-id="5086ac48-b95c-42a9-0a58-6e690b1c648f" style={{
              transform: 'translate3d(0px, 0px, 0px) scale3d(1, 1, 1) rotateX(0deg) rotateY(0deg) rotateZ(0deg) skew(0deg, 0deg)',
              opacity: 1,
              transformStyle: 'preserve-3d'
            }} className="split-content job-post-body">
              <div className="card job-post">
                <div data-w-id="ebcc2b89-9ed8-8b91-2d0e-6dfaa089b244" className="image-wrapper card-job-post"><img
                  src="https://uploads-ssl.webflow.com/637be9fff1765c82ee8ddd63/637be9fff1765c36208dde54_image-7-cover-company-job-board-x-template.jpg"
                  alt="Massachusetts General Hospital"
                  sizes="(max-width: 479px) 90vw, (max-width: 767px) 93vw, (max-width: 991px) 95vw, (max-width: 1439px) 74vw, 846.875px"
                  srcSet="https://uploads-ssl.webflow.com/637be9fff1765c82ee8ddd63/637be9fff1765c36208dde54_image-7-cover-company-job-board-x-template-p-500.jpeg 500w, https://uploads-ssl.webflow.com/637be9fff1765c82ee8ddd63/637be9fff1765c36208dde54_image-7-cover-company-job-board-x-template-p-800.jpeg 800w, https://uploads-ssl.webflow.com/637be9fff1765c82ee8ddd63/637be9fff1765c36208dde54_image-7-cover-company-job-board-x-template-p-1080.jpeg 1080w, https://uploads-ssl.webflow.com/637be9fff1765c82ee8ddd63/637be9fff1765c36208dde54_image-7-cover-company-job-board-x-template.jpg 1964w"
                  className="image card-job-post" style={{
                  willChange: 'transform',
                  transform: 'translate3d(0px, -4.8568px, 0px) scale3d(1.02428, 1.02428, 1) rotateX(0deg) rotateY(0deg) rotateZ(0deg) skew(0deg, 0deg)',
                  transformStyle: 'preserve-3d'
                }}/></div>
                <div className="card-job-post-content">
                  <div className="card-job-post-content-top">
                    <div className="card-job-post-title-wrapper">
                      <div className="image-wrapper card-job-post-logo"><img
                        src="https://uploads-ssl.webflow.com/637be9fff1765c82ee8ddd63/63a413630bd706414486f790_mgh.png"
                        alt="Haplo Peripheral Blood Sct In GVHD Prevention" className="image card-job-post-logo"/></div>
                      <h1 className="title h2-size card-job-post">Haplo Peripheral Blood Sct In GVHD Prevention</h1>
                    </div>
                    <div className="card-job-post-categories-wrapper">
                      <div className="card-job-post-category-wrapper">
                        <div className="card-job-post-category-title-wrapper"><img
                          src="https://uploads-ssl.webflow.com/637be9fef1765c0f958ddd3c/637be9fff1765c98fc8ddd87_icon-1-job-categories-job-board-x-template.svg"
                          alt="Location Icon - Job Board X Webflow Template"
                          className="card-job-post-category-title-icon"/>
                          <div>Location</div>
                        </div>
                        <div className="card-job-post-category-text">Boston, MA</div>
                      </div>
                      <div className="card-job-post-category-wrapper">
                        <div className="card-job-post-category-title-wrapper"><img
                          src="https://uploads-ssl.webflow.com/637be9fef1765c0f958ddd3c/637be9fff1765c510f8ddd88_icon-2-job-categories-job-board-x-template.svg"
                          alt="Graph Icon - Job Board X Webflow Template"
                          className="card-job-post-category-title-icon"/>
                          <div>Conditions</div>
                        </div>
                        <div className="card-job-post-category-text">Lymphoma</div>
                      </div>
                      <div className="card-job-post-category-wrapper">
                        <div className="card-job-post-category-title-wrapper"><img
                          src="https://uploads-ssl.webflow.com/637be9fef1765c0f958ddd3c/637be9fff1765c89f38ddd8f_icon-3-job-categories-job-board-x-template.svg"
                          alt="Portfolio Icon - Job Board X Webflow Template"
                          className="card-job-post-category-title-icon"/>
                          <div>Study Status</div>
                        </div>
                        <div className="card-job-post-category-text">Enrolling</div>
                      </div>
                      <div className="card-job-post-category-wrapper w-condition-invisible">
                        <div className="card-job-post-category-title-wrapper"><img
                          src="https://uploads-ssl.webflow.com/637be9fef1765c0f958ddd3c/637be9fff1765c52ac8ddd8e_icon-4-job-categories-job-board-x-template.svg"
                          alt="Clock Icon - Job Board X Webflow Template"
                          className="card-job-post-category-title-icon"/>
                          <div>Intervention</div>
                        </div>
                        <div className="card-job-post-category-text">Drug</div>
                      </div>
                      <div className="card-job-post-category-wrapper w-condition-invisible">
                        <div className="card-job-post-category-title-wrapper">
                          <div>Age</div>
                        </div>
                        <div className="card-job-post-category-text">18-80</div>
                      </div>
                    </div>
                  </div>
                  <div className="divider card-job-post-content-divider"/>
                  <div className="card-job-post-content-bottom">
                    <div className="job-description-title-wrapper"><h2 className="title h3-size job-description">Trial
                      Overview</h2>
                      <div className="job-description-date-wrapper">
                        <div className="job-description-date-text">Posted on:&nbsp;</div>
                        <div>December 22, 2022</div>
                      </div>
                    </div>
                    <div className="w-richtext"><p>This clinical trial is evaluating the effectiveness of a potential
                      treatment called RGI-2001 in preventing a condition called Graft-vs-Host Disease (GVHD) in
                      patients with various types of blood disorders, such as acute myeloid leukemia (AML), acute
                      lymphoblastic leukemia (ALL), and non-Hodgkin lymphoma (NHL). The study is specifically targeting
                      individuals who are about to undergo a blood stem cell transplantation. GVHD occurs when cells
                      from the donor's tissue attack the recipient's organs. RGI-2001 is an experimental treatment that
                      is being tested in this study. The researchers will be looking at the number of patients who have
                      successful donor engraftment and the 100-day non-relapse mortality rate to determine the safety
                      and effectiveness of RGI-2001 in preventing GVHD. The study is being conducted by the Regimmune
                      Corporation and is taking place at the Massachusetts General Hospital in Boston. It is open to
                      adults and older adults between the ages of 18 and 80. The study is being conducted in a phase 1
                      design, which means that it is an early-stage trial focused on determining the safety and dosage
                      of the treatment.</p></div>
                    <h2 className="title h3-size job-rich-text">Inclusion Criteria</h2>
                    <div className="w-richtext"><p>Men or women ≥ 18 and ≤ 80 years old.</p><p>Diagnosis of
                      hematological malignancy: </p><p>Acute myeloid leukemia (AML) or acute lymphoblastic leukemia
                      (ALL) in morphologic complete remission, Myelodysplastic syndrome (MDS), myeloproliferative
                      disorders (MPN), or chronic myelomonocytic leukemic (CMML) with &lt; 5% blasts in blood or bone
                      marrow.</p><p>Chemosensitive Hodgkin lymphoma (HL) or Non-Hodgkin lymphoma (NHL).</p><p>Patients
                      must be undergoing haploidentical allogeneic hematopoietic cell transplantation, defined as 1st or
                      2nd degree relative with at least 5/10 matching at HLA-A, -B, -C, DR, and DQ.</p><p>ECOG
                      performance status ≤2.</p><p>Patients with adequate physical function as measured by:</p>
                      <p>Cardiac: Left ventricular ejection fraction at rest must be ≥ 40%, or shortening
                        fraction &gt;25%</p><p>Hepatic: Bilirubin ≤ 2.5 mg/dL, except for patients with Gilbert's
                        syndrome or hemolysis</p><p>ALT, AST, and Alkaline Phosphatase &lt; 5 x ULN</p><p>Renal: Serum
                        creatinine within normal range, or if serum creatinine is outside normal range, then renal
                        function (measured or estimated creatinine clearance or GFR) ≥ 40mL/min/1.73m2</p><p>Pulmonary:
                        DLCO (corrected for hemoglobin), FEV1 and FVC ≥ 50% predicted</p><p>Ability to understand and
                        the willingness to sign a written informed consent document</p></div>
                    <h2 className="title h3-size job-rich-text">Exclusion Criteria</h2>
                    <div className="w-richtext"><p>Subjects meeting any of the following criteria are not eligible for
                      participation in the study:</p><p>Recurrent or refractory ALL limited to isolated testicular.</p>
                      <p>Subjects with radiologically detected CNS lymphoma or CNS 3 disease (presence of ≥ 5/µL WBCs in
                        CSF and cytosine positive for blasts [in the absence of a traumatic lumbar puncture] and/or
                        clinical signs of CNS leukemia).</p><p>Hyperleukocytosis (≥ 50,000 blasts/µL) or rapidly
                        progressive disease that in the estimation of the investigator and sponsor would compromise
                        ability to complete study therapy.</p></div>
                    <a href="https://www.massgeneral.org/" onClick={toggleFunc}
                       className="button-primary apply-button w-button">Apply now</a></div>
                </div>
              </div>
              <div data-w-id="bd5ebe39-4c1d-b6ea-1c19-d384bef2b3eb" style={{
                transform: 'translate3d(0px, 20px, 0px) scale3d(1, 1, 1) rotateX(0deg) rotateY(0deg) rotateZ(0deg) skew(0deg, 0deg)',
                opacity: 0,
                transformStyle: 'preserve-3d'
              }} className="job-post-sidebar-wrapper mobile">
                <div className="card job-post-sidebar">
                  <div className="image-wrapper card-job-post-sidebar-icon"><img
                    src="https://uploads-ssl.webflow.com/637be9fef1765c0f958ddd3c/637be9fff1765c18f98ddd9b_icon-apply-job-post-job-board-x-template.svg"
                    alt="Check Icon - Job Board X Webflow Template" className="image card-job-post-sidebar-icon"/></div>
                  <div className="card-job-post-sidebar-content"><h3
                    className="title h4-size card-job-post-sidebar">Apply now</h3><p
                    className="paragraph card-job-post-sidebar">Please let Webflow know that you found this position on
                    our job board, as that is a great way to support us, so we can keep posting cool jobs every day.</p>
                    <a href="#" className="button-primary small card-job-post-sidebar w-button" onClick={toggleFunc}>Apply
                      now</a></div>
                </div>
                <div className="card job-post-sidebar last">
                  <div className="image-wrapper card-job-post-sidebar-icon"><img
                    src="https://uploads-ssl.webflow.com/637be9fff1765c82ee8ddd63/63a413630bd706414486f790_mgh.png"
                    alt="" className="image card-job-post-sidebar-icon"/></div>
                  <div className="card-job-post-sidebar-content">
                    <div className="card-job-post-sidebar-title-about-company-wrapper"><h3
                      className="title h4-size card-job-post-sidebar-about-company">About</h3><h3
                      className="title h4-size card-job-post-sidebar-about-company">Massachusetts General Hospital</h3>
                    </div>
                    <p className="paragraph card-job-post-sidebar">Lorem ipsum dolor sit amet, consectetur adipiscing
                      elit. Sed turpis sit aliquam faucibus fringilla eu.</p><a
                    href="/company/massachusetts-general-hospital"
                    className="button-secondary small card-job-post-sidebar-about-company w-button">View company</a>
                  </div>
                </div>
              </div>
              <div data-w-id="56137bc8-507d-e022-9903-33a938c5d594" style={{
                transform: 'translate3d(0px, 20px, 0px) scale3d(1, 1, 1) rotateX(0deg) rotateY(0deg) rotateZ(0deg) skew(0deg, 0deg)',
                opacity: 0,
                transformStyle: 'preserve-3d'
              }} className="content-top more-jobs">
                <div className="split-content content-top-more-jobs-left"><h2 className="title more-jobs">More
                  trials</h2></div>
                <div className="split-content content-top-more-jobs-right"><a href="/"
                                                                              className="button-secondary w-button">All
                  trials</a></div>
              </div>
              <div className="w-dyn-list">
                <div data-w-id="d9d4094b-ccc8-136e-0804-0353e66dd204" style={{
                  transform: 'translate3d(0px, 20px, 0px) scale3d(1, 1, 1) rotateX(0deg) rotateY(0deg) rotateZ(0deg) skew(0deg, 0deg)',
                  opacity: 0,
                  transformStyle: 'preserve-3d'
                }} role="list" className="jobs-grid w-dyn-items">
                  <div role="listitem" className="job-item w-dyn-item"><a href="/job/nct02841540"
                                                                          className="card job w-inline-block">
                    <div className="split-content card-job-left">
                      <div className="image-wrapper card-job"><img
                        src="https://uploads-ssl.webflow.com/637be9fff1765c82ee8ddd63/63a413630bd706414486f790_mgh.png"
                        alt="A Study of H3B-8800 (RVT-2001) in Participants With Lower Risk Myelodysplastic Syndromes"
                        className="image card-job"/></div>
                      <div className="card-job-title-wrapper"><h3 className="title h6-size card-job">A Study of H3B-8800
                        (RVT-2001) in Participants With Lower Risk Myelodysplastic Syndromes</h3>
                        <div className="card-link-wrapper">
                          <div className="card-link">Massachusetts General Hospital</div>
                          <div className="card-link-arrow">
                            <div className="card-link-arrow-1"/>
                            <div className="card-link-arrow-2"/>
                            <div className="card-link-arrow-3"/>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="split-content card-job-right">
                      <div className="card-job-category-wrapper">
                        <div className="card-job-category-title-wrapper"><img
                          src="https://uploads-ssl.webflow.com/637be9fef1765c0f958ddd3c/637be9fff1765c98fc8ddd87_icon-1-job-categories-job-board-x-template.svg"
                          alt="Location Icon - Job Board X Webflow Template" className="card-job-category-title-icon"/>
                          <div>Location</div>
                        </div>
                        <div className="card-job-category-text">Boston, MA</div>
                      </div>
                      <div className="card-job-category-wrapper">
                        <div className="card-job-category-title-wrapper"><img
                          src="https://uploads-ssl.webflow.com/637be9fef1765c0f958ddd3c/637be9fff1765c510f8ddd88_icon-2-job-categories-job-board-x-template.svg"
                          alt="Graph Icon - Job Board X Webflow Template" className="card-job-category-title-icon"/>
                          <div>Level</div>
                        </div>
                        <div className="card-job-category-text">Phase I</div>
                      </div>
                      <div className="card-job-category-wrapper">
                        <div className="card-job-category-title-wrapper"><img
                          src="https://uploads-ssl.webflow.com/637be9fef1765c0f958ddd3c/637be9fff1765c89f38ddd8f_icon-3-job-categories-job-board-x-template.svg"
                          alt="Portfolio Icon - Job Board X Webflow Template" className="card-job-category-title-icon"/>
                          <div>Study Status</div>
                        </div>
                        <div className="card-job-category-text">Recruiting</div>
                      </div>
                    </div>
                  </a></div>
                  <div role="listitem" className="job-item w-dyn-item"><a href="/job/dasatinib"
                                                                          className="card job w-inline-block">
                    <div className="split-content card-job-left">
                      <div className="image-wrapper card-job"><img
                        src="https://uploads-ssl.webflow.com/637be9fff1765c82ee8ddd63/63a41c49df36e11ab6e91c4c_MD%20Anderson.png"
                        alt="Dasatinib and Venetoclax in Treating Patients With Philadelphia Chromosome Positive"
                        className="image card-job"/></div>
                      <div className="card-job-title-wrapper"><h3 className="title h6-size card-job">Dasatinib and
                        Venetoclax in Treating Patients With Philadelphia Chromosome Positive</h3>
                        <div className="card-link-wrapper">
                          <div className="card-link">MD Anderson Cancer Center</div>
                          <div className="card-link-arrow">
                            <div className="card-link-arrow-1"/>
                            <div className="card-link-arrow-2"/>
                            <div className="card-link-arrow-3"/>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="split-content card-job-right">
                      <div className="card-job-category-wrapper">
                        <div className="card-job-category-title-wrapper"><img
                          src="https://uploads-ssl.webflow.com/637be9fef1765c0f958ddd3c/637be9fff1765c98fc8ddd87_icon-1-job-categories-job-board-x-template.svg"
                          alt="Location Icon - Job Board X Webflow Template" className="card-job-category-title-icon"/>
                          <div>Location</div>
                        </div>
                        <div className="card-job-category-text">Houston, TX</div>
                      </div>
                      <div className="card-job-category-wrapper">
                        <div className="card-job-category-title-wrapper"><img
                          src="https://uploads-ssl.webflow.com/637be9fef1765c0f958ddd3c/637be9fff1765c510f8ddd88_icon-2-job-categories-job-board-x-template.svg"
                          alt="Graph Icon - Job Board X Webflow Template" className="card-job-category-title-icon"/>
                          <div>Level</div>
                        </div>
                        <div className="card-job-category-text">Phase II</div>
                      </div>
                      <div className="card-job-category-wrapper">
                        <div className="card-job-category-title-wrapper"><img
                          src="https://uploads-ssl.webflow.com/637be9fef1765c0f958ddd3c/637be9fff1765c89f38ddd8f_icon-3-job-categories-job-board-x-template.svg"
                          alt="Portfolio Icon - Job Board X Webflow Template" className="card-job-category-title-icon"/>
                          <div>Study Status</div>
                        </div>
                        <div className="card-job-category-text">Recruiting</div>
                      </div>
                    </div>
                  </a></div>
                  <div role="listitem" className="job-item w-dyn-item"><a href="/job/nct04806035"
                                                                          className="card job w-inline-block">
                    <div className="split-content card-job-left">
                      <div className="image-wrapper card-job"><img
                        src="https://uploads-ssl.webflow.com/637be9fff1765c82ee8ddd63/63a4c52e6709522d234dd7fd_tg.png"
                        alt="Study of TG-1801 Alone or in Combination With Ublituximab" className="image card-job"/>
                      </div>
                      <div className="card-job-title-wrapper"><h3 className="title h6-size card-job">Study of TG-1801
                        Alone or in Combination With Ublituximab</h3>
                        <div className="card-link-wrapper">
                          <div className="card-link">TG Therapeutics</div>
                          <div className="card-link-arrow">
                            <div className="card-link-arrow-1"/>
                            <div className="card-link-arrow-2"/>
                            <div className="card-link-arrow-3"/>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="split-content card-job-right">
                      <div className="card-job-category-wrapper">
                        <div className="card-job-category-title-wrapper"><img
                          src="https://uploads-ssl.webflow.com/637be9fef1765c0f958ddd3c/637be9fff1765c98fc8ddd87_icon-1-job-categories-job-board-x-template.svg"
                          alt="Location Icon - Job Board X Webflow Template" className="card-job-category-title-icon"/>
                          <div>Location</div>
                        </div>
                        <div className="card-job-category-text">Boston, MA</div>
                      </div>
                      <div className="card-job-category-wrapper">
                        <div className="card-job-category-title-wrapper"><img
                          src="https://uploads-ssl.webflow.com/637be9fef1765c0f958ddd3c/637be9fff1765c510f8ddd88_icon-2-job-categories-job-board-x-template.svg"
                          alt="Graph Icon - Job Board X Webflow Template" className="card-job-category-title-icon"/>
                          <div>Level</div>
                        </div>
                        <div className="card-job-category-text">Phase II</div>
                      </div>
                      <div className="card-job-category-wrapper">
                        <div className="card-job-category-title-wrapper"><img
                          src="https://uploads-ssl.webflow.com/637be9fef1765c0f958ddd3c/637be9fff1765c89f38ddd8f_icon-3-job-categories-job-board-x-template.svg"
                          alt="Portfolio Icon - Job Board X Webflow Template" className="card-job-category-title-icon"/>
                          <div>Study Status</div>
                        </div>
                        <div className="card-job-category-text">Enrolling</div>
                      </div>
                    </div>
                  </a></div>
                  <div role="listitem" className="job-item w-dyn-item"><a href="/job/asciminib"
                                                                          className="card job w-inline-block">
                    <div className="split-content card-job-left">
                      <div className="image-wrapper card-job"><img
                        src="https://uploads-ssl.webflow.com/637be9fff1765c82ee8ddd63/63a4addd16e186414f16bc3a_AU.png"
                        alt="Asciminib as Initial Therapy for Patients With Chronic Myeloid Leukemia in Chronic Phase"
                        className="image card-job"/></div>
                      <div className="card-job-title-wrapper"><h3 className="title h6-size card-job">Asciminib as
                        Initial Therapy for Patients With Chronic Myeloid Leukemia in Chronic Phase</h3>
                        <div className="card-link-wrapper">
                          <div className="card-link">Augusta University</div>
                          <div className="card-link-arrow">
                            <div className="card-link-arrow-1"/>
                            <div className="card-link-arrow-2"/>
                            <div className="card-link-arrow-3"/>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="split-content card-job-right">
                      <div className="card-job-category-wrapper">
                        <div className="card-job-category-title-wrapper"><img
                          src="https://uploads-ssl.webflow.com/637be9fef1765c0f958ddd3c/637be9fff1765c98fc8ddd87_icon-1-job-categories-job-board-x-template.svg"
                          alt="Location Icon - Job Board X Webflow Template" className="card-job-category-title-icon"/>
                          <div>Location</div>
                        </div>
                        <div className="card-job-category-text">Augusta, GA</div>
                      </div>
                      <div className="card-job-category-wrapper">
                        <div className="card-job-category-title-wrapper"><img
                          src="https://uploads-ssl.webflow.com/637be9fef1765c0f958ddd3c/637be9fff1765c510f8ddd88_icon-2-job-categories-job-board-x-template.svg"
                          alt="Graph Icon - Job Board X Webflow Template" className="card-job-category-title-icon"/>
                          <div>Level</div>
                        </div>
                        <div className="card-job-category-text">Phase II</div>
                      </div>
                      <div className="card-job-category-wrapper">
                        <div className="card-job-category-title-wrapper"><img
                          src="https://uploads-ssl.webflow.com/637be9fef1765c0f958ddd3c/637be9fff1765c89f38ddd8f_icon-3-job-categories-job-board-x-template.svg"
                          alt="Portfolio Icon - Job Board X Webflow Template" className="card-job-category-title-icon"/>
                          <div>Study Status</div>
                        </div>
                        <div className="card-job-category-text">Enrolling</div>
                      </div>
                    </div>
                  </a></div>
                </div>
              </div>
            </div>
            <div data-w-id="303e2183-3b6a-8bf1-f737-049bfd79a52e" style={{
              transform: 'translate3d(0px, 0px, 0px) scale3d(1, 1, 1) rotateX(0deg) rotateY(0deg) rotateZ(0deg) skew(0deg, 0deg)',
              opacity: 1,
              transformStyle: 'preserve-3d'
            }} className="split-content job-post-sidebar">
              <div className="job-post-sidebar-wrapper">
                <div className="card job-post-sidebar">
                  <div className="image-wrapper card-job-post-sidebar-icon"><img
                    src="https://uploads-ssl.webflow.com/637be9fef1765c0f958ddd3c/637be9fff1765c18f98ddd9b_icon-apply-job-post-job-board-x-template.svg"
                    alt="Check Icon - Job Board X Webflow Template" className="image card-job-post-sidebar-icon"/></div>
                  <div className="card-job-post-sidebar-content"><h3
                    className="title h4-size card-job-post-sidebar">Check Your Eligibility</h3><p
                    className="paragraph card-job-post-sidebar">Check your eligibility for this trial with our digital
                    screener. It's fast and easy to get started!</p></div>
                  <a href="https://www.massgeneral.org/"
                     className="button-primary small card-job-post-sidebar w-button" onClick={toggleFunc}>Check my eligibility </a></div>
                <div className="card job-post-sidebar last">
                  <div className="image-wrapper card-job-post-sidebar-icon"><img
                    src="https://uploads-ssl.webflow.com/637be9fff1765c82ee8ddd63/63a413630bd706414486f790_mgh.png"
                    alt="Massachusetts General Hospital" className="image card-job-post-sidebar-icon"/></div>
                  <div className="card-job-post-sidebar-content">
                    <div className="card-job-post-sidebar-title-about-company-wrapper"><h3
                      className="title h4-size card-job-post-sidebar-about-company">About</h3><h3
                      className="title h4-size card-job-post-sidebar-about-company">Massachusetts General Hospital</h3>
                    </div>
                    <p className="paragraph card-job-post-sidebar">Lorem ipsum dolor sit amet, consectetur adipiscing
                      elit. Sed turpis sit aliquam faucibus fringilla eu.</p></div>
                  <a href="/company/massachusetts-general-hospital"
                     className="button-secondary small card-job-post-sidebar-about-company w-button">View company</a>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>

      <Footer/>
    </>
  )
}
